...
void DeviceManager::updateDeivceUsage(std::shared_ptr<Device> d) {
...
void DeviceManager::setTime(Time time) {
  ...
  while (currentTime != time) {
    ...
    for (const auto& device : devices){
      if ( device->getIsProgramValid() && device->getStart() == currentTime )
        DeviceManager::turnOnDevice(device);

      if ( device->getIsProgramValid() && device->getFinish() == currentTime )
        DeviceManager::turnOffDevice(device);

      if ( device->isFinishProgramValid() && device->getFinishProgram() == currentTime )
        DeviceManager::turnOffDevice(device);
    }
  }
}
