...
private:
  ...
  MyTime currentTime;     // Simulated current time (initialized to 00:00)
...
public:
  ...
  MyTime getCurrentTime() { return currentTime; }
  ...
  void setStartTimer( std::shared_ptr<Device> d, const MyTime& time );

  //This schedules when the device should turn off
  void setStopTimer( std::shared_ptr<Device> d, const MyTime& time );
  ...
  std::vector<std::string> setTime(MyTime time);
